...
class OrdersAdminController {
    ...
    async getShipperOrders(req, res){
        try {
            const {shipperId} = req.query;
            const orders = await Orders.find({$or: [{ _shipperId: shipperId }, { _status: 0 }]});
            res.json(orders);
        }
        catch (error) {
            console.error('Lỗi khi lấy danh sách đơn hàng!');
    ...
    async comfirmOrder(req, res) {
        try {
            ...
            const data = req.body;
            console.log(data);
            const updateData = {$set: {_status:data._status, _shipperId: data._shipperId}};
            const result = await Orders.updateOne(query, updateData);
            res.json(result.matchedCount);
        }catch (error) {
            console.error('Lỗi khi cập nhật đơn hàng!');
